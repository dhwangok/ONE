...
class Learn():
    ...
    def train(self, path=None, model_path=None, X=None, y=None, clf=None, algorithm='LogisticRegression'):
        ...
        est = GradientBoostingRegressor(n_estimators=500,
                                        ...
                                        max_depth=5,
                                        random_state=1,
                                        loss='huber').fit(X_train, y_train)  # 'huber'的效果最好  ‘ls’, ‘lad’,# ‘huber’, ‘quantile’ #3,500,0.01
        ...
        # est=ExtraTreesRegressor(n_estimators=10, criterion='mse', max_depth=None, min_samples_split=2, min_samples_leaf=1, min_weight_fraction_leaf=0.0,
        ...
        #                             max_features='auto', max_leaf_nodes=None, bootstrap=True, oob_score=False, n_jobs=1, random_state=None, verbose=0, warm_start=False).fit(X_train, y_train)
...
def PreprocessData(featureFile, path):
    ...
    open('C:/danhua/carset_ex_1/SVR/id_Set/model_id_set_1.csv',
    ...
    open('C:/danhua/carset_ex_1/SVR/id_Set/prov_id_set_1.csv',
         'wb').write(fea_prov_set)

    for city_id_item in city_set:
        fea_city_set += city_id_item + '\n'
    open('C:/danhua/carset_ex_1/SVR/id_Set/city_id_set_1.csv',
    ...
    while line:
        ...
        d2 = datetime.datetime(int(year), int(month), int(day))
        reg_date_day = (d1 - d2).days
        reg_date_type=1.0*reg_date_day/365
        
        print reg_date_day,reg_type
        ...
        instance +=gear_type+','
        instance += str(model_year_type)+ ','          #str(int(model_year)-2000) + ','
        instance += model_price + ','
        instance += liter + ','
        # instance+=car_status+','
        # instance+=str(1.0*reg_date_day/max_reg_day)+','
        instance+=str(reg_type)+','
...
if __name__ == '__main__':
    ...
    start = time.clock()
    # "C:/danhua/carset_ex_1/train_data_21.csv"
    # prov/city/train_data_city2.csv
    raw_data_path ="C:/danhua/carset_ex_1/carset_ex_1_0810/carset_ex_1_gear_drFalse2008.csv"
    featureFile = 'C:/danhua/carset_ex_1/SVR/fea/fea_carset_1_gbdt_int_gear_new.csv'

    PreprocessData(featureFile, raw_data_path)
    ln = Learn()
    ln.load_data(featureFile)
    ln.train(path=featureFile,
             model_path='C:/danhua/carset_ex_1/SVR/model/carset_1_gbdt_int_gear_new.model')
