...
def splitLine(dt):
    ...
    price = dt[5]
    score_weight=dt[13]
    return car_id, model_id, prov_id, city_id, reg_date, car_source, car_status, mile_age, post_time, liter, model_year, model_price, price, reg_year,score_weight


#连接数据库拿到对应model_id的NAME,年款,指导价
def model_name(i):
...
#从数据库里获得车系里model_id和相应的gear_type,，手动为0，自动为1，电动-1
def gear(i):
    conn = pymysql.connect(host="192.168.0.225", user="majian",
                           passwd="MjAn@9832#", db="myche", charset="utf8")
    cursor = conn.cursor()
...
def getData(path,i):
    f = open(path) 
    car_id_data, model_data, prov_data, city_data, reg_data, car_source_data, car_status_data, mile_data, post_data, liter_data, model_year_data, model_price_data, price_data,score_weight_data = [
    ], [], [], [], [], [], [], [], [], [], [], [], [],[]
    ...
    while line:
        dt = line.split('	')
        # dt = line.split(',')
        car_id, model_id, prov_id, city_id, reg_date, car_source, car_status, mile_age, post_time, liter, model_year, model_price, price, reg_year,score_weight = splitLine(
            dt)
        ...
        score_weight_data.append(score_weight)
        line = f.readline().strip()
        # else:
        #     line = f.readline().strip()

    df = pd.DataFrame([car_id_data, model_data, prov_data, city_data, reg_data, price_data, car_source_data, car_status_data, mile_data,
                       post_data, liter_data, model_year_data, model_price_data,score_weight_data]).T
    #定义列名
    df = df.rename(columns={0: "car_id", 1: "model_id", 2: "prov_id", 3: "city_id", 4: "reg_date", 5: "report_price", 6: "car_source", 7: "car_status",
                            8: "mile_age", 9: "post_time", 10: "liter", 11: "model_year", 12: "model_price",13:"score_weight"})
    #保存文件
    df.to_csv("D:/data/model_carset_1_11.csv/model_carset_%s_11_1.csv"% i, index=False)  
...
if __name__ == '__main__':
    ...
    while i < 2:  
        try:
            path="D:/data/model_carset_1_11.csv/model_carset_1_11.csv"   #'D:/data/carset_ex_0818_2/carset_ex/carset_ex_%s.csv'%i     
            print i
            f=pd.read_csv(path)
            if f.shape[0] !=0:
                getData(path,i)
