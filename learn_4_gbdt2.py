...
def PreprocessData(featureFile,path,j):
	...
	data=[]
	feature=''
	d1 = datetime.datetime(2016, 4, 23)
	...
	open('C:/danhua/code/fea/model_id_set_%s.csv'%j,'wb').write(fea_model_id)
	...
	open('C:/danhua/code/fea/prov_id_set_%s.csv'%j,'wb').write(fea_prov_set)

	for city_id_item in city_set:
		fea_city_set+=city_id_item+'\n'
	open('C:/danhua/code/fea/city_id_set_%s.csv'%j,'wb').write(fea_city_set)
...
if __name__=='__main__':
	...
	while j <409:
		try:
			print j
			raw_data_path="C:/danhua/code/analysis/exact_data_0.3/exact_data_%s.csv"%j
			featureFile='C:/danhua/code/feature/fea_da_car_series_%s.csv'%j

			PreprocessData(featureFile,raw_data_path,j)
			ln=Learn()
			ln.load_data(featureFile)
			ln.train(path=featureFile,model_path='C:/danhua/code/model/da_car_series_%s.model'%j)
			j+=1
